...
PUBLIC_APP_URL = os.environ.get(
    "PUBLIC_APP_URL",
    "https://academic-genealogy-inf.streamlit.app/",
).strip().rstrip("/")
...
THEMATIC_CLASSIFIER: List[ClassifierItem] = [
    ...
    ("1.1.1.2", "Вычислительные машины и комплексы", False),
    ("1.1.1.3", "Многопроцессорные и параллельные системы", False),
    ("1.1.1.4", "Встраиваемые и киберфизические системы", False),

    # 1.1.2. Компьютерные сети
    ("1.1.2", "Компьютерные сети", True),
    ("1.1.2.1", "Локальные и корпоративные сети", False),
    ("1.1.2.2", "Глобальные сети и Интернет", False),
    ("1.1.2.3", "Беспроводные и мобильные сети", False),
    ("1.1.2.4", "Промышленные сети и Интернет вещей (IoT/IIoT)", False),

    # 1.1.3. Системы управления
    ("1.1.3", "Системы управления", True),
    ("1.1.3.1", "АСУТП (управление технологическими процессами)", False),
    ("1.1.3.2", "АСУП (управление производствами)", False),
    ("1.1.3.3", "Системы управления в организационных системах", False),
    ("1.1.3.4", "Интеллектуальные системы управления", False),

    # 1.1.4. Информационные системы
    ("1.1.4", "Информационные системы", True),
    ("1.1.4.1", "Системы управления базами данных и знаний", False),
    ("1.1.4.2", "Распределенные информационные системы", False),
    ("1.1.4.3", "Экспертные системы и СППР", False),
    ("1.1.4.4", "Информационно-поисковые и справочные системы", False),

    # 1.1.5. Программные системы
    ("1.1.5", "Программные системы", True),
    ("1.1.5.1", "Операционные системы", False),
    ("1.1.5.2", "Системы программирования и инструментальные средства", False),
    ("1.1.5.3", "САПР/CAD/CAE/BIM системы", False),
    ("1.1.5.4", "Корпоративные системы (ERP/MES/CRM)", False),

    # 1.1.6. Системы информационной безопасности
    ("1.1.6", "Системы информационной безопасности", True),
    ("1.1.6.1", "Системы защиты информации", False),
    ("1.1.6.2", "Криптографические системы", False),
    ("1.1.6.3", "Системы обнаружения и предотвращения вторжений", False),
    ("1.1.6.4", "Системы аутентификации и управления доступом", False),

    # 1.2. Типы данных и информации
    ("1.2", "Типы данных и информации", True),

    # 1.2.1. Структурированные данные
    ("1.2.1", "Структурированные данные", True),
    ("1.2.1.1", "Реляционные и табличные данные", False),
    ("1.2.1.2", "Графовые данные и онтологии", False),
    ("1.2.1.3", "Временные ряды и телеметрия", False),

    # 1.2.2. Мультимедийные данные
    ("1.2.2", "Мультимедийные данные", True),
    ("1.2.2.1", "Изображения и видеопоток", False),
    ("1.2.2.2", "Аудио и речевые сигналы", False),
    ("1.2.2.3", "Пространственные и геоданные (ГИС)", False),

    # 1.2.3. Текстовые данные
    ("1.2.3", "Текстовые данные", True),
    ("1.2.3.1", "Документы и корпуса текстов", False),
    ("1.2.3.2", "Естественноязычные данные (NLP)", False),
    ("1.2.3.3", "Программный код и спецификации", False),

    # 1.2.4. Знания и экспертные данные
    ("1.2.4", "Знания и экспертные данные", True),
    ("1.2.4.1", "Базы знаний и правила", False),
    ("1.2.4.2", "Экспертные оценки", False),
    ("1.2.4.3", "Большие данные (Big Data)", False),

    # 1.3. Отрасли и сферы применения
    ("1.3", "Отрасли и сферы применения", True),

    # 1.3.1. Промышленность и производство
    ("1.3.1", "Промышленность и производство", True),
    ("1.3.1.1", "Машиностроение и металлургия", False),
    ("1.3.1.2", "Нефтегазовая отрасль и энергетика", False),
    ("1.3.1.3", "Химическая и пищевая промышленность", False),
    ("1.3.1.4", "Строительство и ЖКХ", False),

    # 1.3.2. Транспорт и логистика
    ("1.3.2", "Транспорт и логистика", True),
    ("1.3.2.1", "Авиационный и космический транспорт", False),
    ("1.3.2.2", "Автомобильный и железнодорожный транспорт", False),
    ("1.3.2.3", "Морской и речной транспорт", False),
    ("1.3.2.4", "Логистические системы и цепи поставок", False),

    # 1.3.3. Телекоммуникации и связь
    ("1.3.3", "Телекоммуникации и связь", True),
    ("1.3.3.1", "Сети передачи данных", False),
    ("1.3.3.2", "Мобильная и спутниковая связь", False),
    ("1.3.3.3", "Телекоммуникационные протоколы и сервисы", False),

    # 1.3.4. Социально-экономическая сфера
    ("1.3.4", "Социально-экономическая сфера", True),
    ("1.3.4.1", "Финансово-экономические системы", False),
    ("1.3.4.2", "Государственное управление", False),
    ("1.3.4.3", "Здравоохранение и медицина", False),

    # 1.3.5. Образование и наука
    ("1.3.5", "Образование и наука", True),
    ("1.3.5.1", "Образование", False),
    ("1.3.5.2", "Научные исследования", False),
    ("1.3.5.3", "Наукометрия", False),

    # 1.3.6. Оборона и безопасность
    ("1.3.6", "Оборона и безопасность", True),
    ("1.3.6.1", "Критическая инфраструктура", False),
    ("1.3.6.2", "Военные и специальные системы", False),
    ("1.3.6.3", "Системы общественной безопасности", False),

    # 2. Методы, технологии и процессы
    ("2", "Методы, технологии и процессы", True),

    # 2.1. Системный анализ и моделирование
    ("2.1", "Системный анализ и моделирование", True),

    # 2.1.1. Методы системного анализа
    ("2.1.1", "Методы системного анализа", True),
    ("2.1.1.1", "Теоретико-множественный и информационный анализ", False),
    ("2.1.1.2", "Структурно-функциональный анализ", False),
    ("2.1.1.3", "Формализация и постановка задач", False),
    ("2.1.1.4", "Идентификация систем", False),

    # 2.1.2. Математическое моделирование
    ("2.1.2", "Математическое моделирование", True),
    ("2.1.2.1", "Аналитические и численные методы", False),
    ("2.1.2.2", "Стохастические и детерминированные модели", False),
    ("2.1.2.3", "Статистические модели и анализ данных", False),
    ("2.1.2.4", "Модели массового обслуживания и надежности", False),

    # 2.1.3. Имитационное моделирование
    ("2.1.3", "Имитационное моделирование", True),
    ("2.1.3.1", "Дискретно-событийное моделирование", False),
    ("2.1.3.2", "Агентное и системно-динамическое моделирование", False),
    ("2.1.3.3", "Цифровые двойники", False),
    ("2.1.3.4", "Компьютерный и вычислительный эксперимент", False),

    # 2.2. Теория управления и оптимизация
    ("2.2", "Теория управления и оптимизация", True),

    # 2.2.1. Методы управления
    ("2.2.1", "Методы управления", True),
    ("2.2.1.1", "Оптимальное и адаптивное управление", False),
    ("2.2.1.2", "Робастное и нечеткое управление", False),
    ("2.2.1.3", "Интеллектуальное управление", False),
    ("2.2.1.4", "Управление в реальном времени", False),

    # 2.2.2. Методы оптимизации
    ("2.2.2", "Методы оптимизации", True),
    ("2.2.2.1", "Линейное и нелинейное программирование", False),
    ("2.2.2.2", "Динамическое и целочисленное программирование", False),
    ("2.2.2.3", "Многокритериальная оптимизация", False),
    ("2.2.2.4", "Стохастическая оптимизация", False),

    # 2.2.3. Эволюционные методы
    ("2.2.3", "Эволюционные методы", True),
    ("2.2.3.1", "Генетические алгоритмы", False),
    ("2.2.3.2", "Роевой интеллект и эволюционные стратегии", False),
    ("2.2.3.3", "Метаэвристические алгоритмы", False),

    # 2.2.4. Принятие решений
    ("2.2.4", "Принятие решений", True),
    ("2.2.4.1", "Методы принятия решений в условиях неопределенности", False),
    ("2.2.4.2", "Экспертные оценки и групповое принятие решений", False),
    ("2.2.4.3", "Интеллектуальная поддержка принятия решений", False),
    ("2.2.4.4", "Теория игр и конфликтов", False),

    # 2.3. Обработка и анализ данных
    ("2.3", "Обработка и анализ данных", True),

    # 2.3.1. Статистические методы
    ("2.3.1", "Статистические методы", True),
    ("2.3.1.1", "Статистический и корреляционный анализ", False),
    ("2.3.1.2", "Регрессионный анализ", False),
    ("2.3.1.3", "Анализ временных рядов", False),
    ("2.3.1.4", "Байесовские методы", False),

    # 2.3.2. Интеллектуальный анализ данных
    ("2.3.2", "Интеллектуальный анализ данных", True),
    ("2.3.2.1", "Data Mining и обнаружение закономерностей", False),
    ("2.3.2.2", "Кластерный анализ и классификация", False),
    ("2.3.2.3", "Ассоциативный анализ", False),
    ("2.3.2.4", "OLAP и многомерный анализ", False),

    # 2.3.3. Обработка сигналов и медиа
    ("2.3.3", "Обработка сигналов и медиа", True),
    ("2.3.3.1", "Цифровая обработка сигналов", False),
    ("2.3.3.2", "Обработка изображений и видео", False),
    ("2.3.3.3", "Распознавание и синтез речи", False),
    ("2.3.3.4", "Извлечение признаков", False),

    # 2.3.4. Обработка текстов
    ("2.3.4", "Обработка текстов", True),
    ("2.3.4.1", "Обработка естественного языка (NLP)", False),
    ("2.3.4.2", "Извлечение информации из текстов", False),
    ("2.3.4.3", "Семантический и синтаксический анализ", False),
    ("2.3.4.4", "Генерация текстов", False),

    # 2.4. Искусственный интеллект и машинное обучение
    ("2.4", "Искусственный интеллект и машинное обучение", True),

    # 2.4.1. Методы машинного обучения
    ("2.4.1", "Методы машинного обучения", True),
    ("2.4.1.1", "Обучение с учителем", False),
    ("2.4.1.2", "Обучение без учителя", False),
    ("2.4.1.3", "Обучение с подкреплением", False),
    ("2.4.1.4", "Ансамблевые методы", False),

    # 2.4.2. Нейросетевые технологии
    ("2.4.2", "Нейросетевые технологии", True),
    ("2.4.2.1", "Сверточные нейронные сети (CNN)", False),
    ("2.4.2.2", "Рекуррентные нейронные сети (RNN/LSTM)", False),
    ("2.4.2.3", "Трансформеры и языковые модели", False),
    ("2.4.2.4", "Генеративные модели (GAN, VAE)", False),

    # 2.4.3. Интеллектуальные технологии
    ("2.4.3", "Интеллектуальные технологии", True),
    ("2.4.3.1", "Распознавание образов", False),
    ("2.4.3.2", "Компьютерное зрение", False),
    ("2.4.3.3", "Нечеткая логика и мягкие вычисления", False),
    ("2.4.3.4", "Когнитивное моделирование", False),

    # 2.4.4. Работа со знаниями
    ("2.4.4", "Работа со знаниями", True),
    ("2.4.4.1", "Представление знаний (онтологии, семантические сети)", False),
    ("2.4.4.2", "Логический вывод и рассуждения", False),
    ("2.4.4.3", "Извлечение и приобретение знаний", False),
    ("2.4.4.4", "Экспертные системы", False),

    # 2.5. Проектирование и разработка
    ("2.5", "Проектирование и разработка", True),

    # 2.5.1. Методы проектирования систем
    ("2.5.1", "Методы проектирования систем", True),
    ("2.5.1.1", "Архитектурное проектирование", False),
    ("2.5.1.2", "Структурно-параметрический синтез", False),
    ("2.5.1.3", "Объектно-ориентированное проектирование", False),
    ("2.5.1.4", "Автоматизация проектных процедур (САПР)", False),

    # 2.5.2. Разработка программного обеспечения
    ("2.5.2", "Разработка программного обеспечения", True),
    ("2.5.2.1", "Проектирование и анализ программ", False),
    ("2.5.2.2", "Языки программирования и трансляция", False),
    ("2.5.2.3", "Верификация и тестирование ПО", False),
    ("2.5.2.4", "Разработка безопасного ПО", False),

    # 2.5.3. Технологии реализации
    ("2.5.3", "Технологии реализации", True),
    ("2.5.3.1", "Параллельное и распределенное программирование", False),
    ("2.5.3.2", "Облачные и граничные вычисления", False),
    ("2.5.3.3", "Микросервисная и сервисно-ориентированная архитектура", False),
    ("2.5.3.4", "Контейнеризация и виртуализация", False),

    # 2.5.4. Визуализация и интерфейсы
    ("2.5.4", "Визуализация и интерфейсы", True),
    ("2.5.4.1", "Компьютерная графика и 3D-моделирование", False),
    ("2.5.4.2", "Визуализация данных", False),
    ("2.5.4.3", "Виртуальная и дополненная реальность (VR/AR)", False),
    ("2.5.4.4", "Человеко-машинные интерфейсы", False),

    # 2.6. Обеспечение информационной безопасности
    ("2.6", "Обеспечение информационной безопасности", True),

    # 2.6.1. Анализ угроз и рисков
    ("2.6.1", "Анализ угроз и рисков", True),
    ("2.6.1.1", "Выявление и классификация угроз", False),
    ("2.6.1.2", "Анализ уязвимостей", False),
    ("2.6.1.3", "Оценка рисков", False),
    ("2.6.1.4", "Моделирование атак", False),

    # 2.6.2. Криптографические методы
    ("2.6.2", "Криптографические методы", True),
    ("2.6.2.1", "Симметричная криптография", False),
    ("2.6.2.2", "Асимметричная криптография", False),
    ("2.6.2.3", "Криптографические протоколы", False),
    ("2.6.2.4", "Квантовая криптография", False),

    # 2.6.3. Методы защиты
    ("2.6.3", "Методы защиты", True),
    ("2.6.3.1", "Идентификация и аутентификация", False),
    ("2.6.3.2", "Разграничение и контроль доступа", False),
    ("2.6.3.3", "Обнаружение и предотвращение вторжений", False),
    ("2.6.3.4", "Защита от вредоносного ПО", False),

    # 2.6.4. Управление безопасностью
    ("2.6.4", "Управление безопасностью", True),
    ("2.6.4.1", "Политика информационной безопасности", False),
    ("2.6.4.2", "Мониторинг и аудит безопасности", False),
    ("2.6.4.3", "Управление инцидентами", False),
    ("2.6.4.4", "Обеспечение непрерывности функционирования", False),

    # 3. Результаты и целевые характеристики
    ("3", "Результаты и целевые характеристики", True),

    # 3.1. Виды научных результатов
    ("3.1", "Виды научных результатов", True),

    # 3.1.1. Теоретические результаты
    ("3.1.1", "Теоретические результаты", True),
    ("3.1.1.1", "Математические модели", False),
    ("3.1.1.2", "Методы и алгоритмы", False),
    ("3.1.1.3", "Теоретические положения и концепции", False),
    ("3.1.1.4", "Критерии оценки эффективности", False),

    # 3.1.2. Архитектурно-структурные решения
    ("3.1.2", "Архитектурно-структурные решения", True),
    ("3.1.2.1", "Архитектуры систем", False),
    ("3.1.2.2", "Структуры и топологии", False),
    ("3.1.2.3", "Протоколы и интерфейсы", False),
    ("3.1.2.4", "Организация взаимодействия компонентов", False),

    # 3.1.3. Программное обеспечение
    ("3.1.3", "Программное обеспечение", True),
    ("3.1.3.1", "Системное программное обеспечение", False),
    ("3.1.3.2", "Прикладное программное обеспечение", False),
    ("3.1.3.3", "Пакеты и комплексы программ", False),
    ("3.1.3.4", "Программные библиотеки и модули", False),

    # 3.1.4. Информационное обеспечение
    ("3.1.4", "Информационное обеспечение", True),
    ("3.1.4.1", "Модели и структуры данных", False),
    ("3.1.4.2", "Базы данных и знаний", False),
    ("3.1.4.3", "Словари и онтологии", False),
    ("3.1.4.4", "Наборы данных (Datasets)", False),

    # 3.1.5. Технические и аппаратные решения
    ("3.1.5", "Технические и аппаратные решения", True),
    ("3.1.5.1", "Аппаратные устройства и элементы", False),
    ("3.1.5.2", "Экспериментальные стенды", False),
    ("3.1.5.3", "Прототипы систем", False),
    ("3.1.5.4", "Технические спецификации", False),

    # 3.1.6. Методическое обеспечение
    ("3.1.6", "Методическое обеспечение", True),
    ("3.1.6.1", "Методики и регламенты", False),
    ("3.1.6.2", "Стандарты и нормативы", False),
    ("3.1.6.3", "Рекомендации по применению", False),
    ("3.1.6.4", "Инструментальные средства", False),

    # 3.2. Функциональные характеристики
    ("3.2", "Функциональные характеристики", True),

    # 3.2.1. Решаемые задачи управления
    ("3.2.1", "Решаемые задачи управления", True),
    ("3.2.1.1", "Идентификация и диагностика", False),
    ("3.2.1.2", "Прогнозирование и планирование", False),
    ("3.2.1.3", "Оптимизация и регулирование", False),
    ("3.2.1.4", "Мониторинг и контроль", False),

    # 3.2.2. Решаемые задачи обработки информации
    ("3.2.2", "Решаемые задачи обработки информации", True),
    ("3.2.2.1", "Сбор и накопление данных", False),
    ("3.2.2.2", "Хранение и поиск информации", False),
    ("3.2.2.3", "Передача и распространение данных", False),
    ("3.2.2.4", "Анализ и интерпретация данных", False),

    # 3.2.3. Решаемые задачи проектирования
    ("3.2.3", "Решаемые задачи проектирования", True),
    ("3.2.3.1", "Синтез структуры", False),
    ("3.2.3.2", "Параметрическая оптимизация", False),
    ("3.2.3.3", "Верификация и валидация", False),
    ("3.2.3.4", "Документирование", False),

    # 3.2.4. Решаемые задачи защиты
    ("3.2.4", "Решаемые задачи защиты", True),
    ("3.2.4.1", "Обнаружение угроз", False),
    ("3.2.4.2", "Предотвращение атак", False),
    ("3.2.4.3", "Восстановление после инцидентов", False),
    ("3.2.4.4", "Обеспечение устойчивости", False),

    # 3.3. Показатели эффективности
    ("3.3", "Показатели эффективности", True),

    # 3.3.1. Производительность
    ("3.3.1", "Производительность", True),
    ("3.3.1.1", "Быстродействие и время отклика", False),
    ("3.3.1.2", "Пропускная способность", False),
    ("3.3.1.3", "Масштабируемость", False),
    ("3.3.1.4", "Параллелизм и распределенность", False),

    # 3.3.2. Точность и качество
    ("3.3.2", "Точность и качество", True),
    ("3.3.2.1", "Точность вычислений и измерений", False),
    ("3.3.2.2", "Достоверность результатов", False),
    ("3.3.2.3", "Полнота и адекватность", False),
    ("3.3.2.4", "Сходимость алгоритмов", False),

    # 3.3.3. Ресурсоемкость
    ("3.3.3", "Ресурсоемкость", True),
    ("3.3.3.1", "Вычислительная сложность", False),
    ("3.3.3.2", "Объем памяти", False),
    ("3.3.3.3", "Энергопотребление", False),
    ("3.3.3.4", "Стоимость решения", False),

    # 3.3.4. Надежность
    ("3.3.4", "Надежность", True),
    ("3.3.4.1", "Безотказность функционирования", False),
    ("3.3.4.2", "Отказоустойчивость и восстанавливаемость", False),
    ("3.3.4.3", "Живучесть системы", False),
    ("3.3.4.4", "Помехоустойчивость", False),

    # 3.3.5. Безопасность
    ("3.3.5", "Безопасность", True),
    ("3.3.5.1", "Защищенность от угроз", False),
    ("3.3.5.2", "Конфиденциальность данных", False),
    ("3.3.5.3", "Целостность информации", False),
    ("3.3.5.4", "Доступность сервисов", False),

    # 3.3.6. Практическая применимость
    ("3.3.6", "Практическая применимость", True),
    ("3.3.6.1", "Автоматизация процессов", False),
    ("3.3.6.2", "Адаптивность к условиям", False),
    ("3.3.6.3", "Интеграция с существующими системами", False),
    ("3.3.6.4", "Удобство использования (Usability)", False),
]
...
with header_left:
    ...
    st.caption(
        "Платформа для построения деревьев научного руководства, поиска и сравнения диссертаций по содержательным и формальным критериям. В настоящий момент основу базы данных составляют диссертационные исследования по техническим наукам (группа специальностей «Информационные технологии и телекоммуникации») с 2005 по 2025 год."
    )
with header_right:
    feedback_button()
